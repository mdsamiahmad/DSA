...
TOPICS = {
    Array [
        ...
        Subarray, Max, Binary-Subarrays, Running-Sum,
        Row-With-Maximum-Ones, Sorting, Missing, Repeating
    ],
    Binary Search [
    ...
    Matrix [
        2D-Matrix, Peak-Element-II, Row-With-Maximum-Ones
    ],
    Dynamic Programming [
...
[LeetCode Profile](httpsleetcode.comumdsamiahmad)
...
def generate_readme()
    ...
    with open(README.md, w, encoding=utf-8) as f
        ...
        for topic in sorted(categorized.keys())
            f.write(fn### {topic}nn)
            f.write( Problems n---n)

            for folder in categorized[topic]
                url = fhttpsgithub.commdsamiahmadDSAtreemain{folder}
                f.write(f [{folder}]({url}) n)
